...
exports.searchCollege = async (filters, username) => {
    ...
    try {
        ...
        if (filters.name) criteria.Name = { [Op.substring]: filters.name };
        ...
        if (filters.sizeMin && filters.sizeMax) criteria.Size = { [Op.between]: [filters.sizeMin, filters.sizeMax] };
        ...
        if (filters.sortAttribute && filters.sortDirection) query.order = [[filters.sortAttribute, filters.sortDirection]];
        ...
        if (filters.major && filters.major2) {
            for (let i = searchResults.length - 1; i >= 0; i--) {
                ...
                for (let j = 0; j < majors.length; j++) {
                    ...
                    if (majors[j].Major.toLowerCase().includes(filters.major.toLowerCase())) { flag1 = true; }
                    if (majors[j].Major.toLowerCase().includes(filters.major2.toLowerCase())) flag2 = true;
                }
        ...
        if (filters.costMax) {
            ...
            for (let i = searchResults.length - 1; i >= 0; i--) {
                ...
                else if (searchResults[i].CostOfAttendanceOutOfState > filters.costMax) searchResults.splice(i, 1);
            }
        }
    } catch (error) {
        return {
